...
const AILogic = (function() {
    ...
    function fenXiShouPai(shouPai) {
        ...
        // 顺子（★ 2026-09-21 对齐官方: **恰好 5 张** ✓ 原来 len>=5 ✗ 会造出游戏不认的 6 张顺子）
        const danList = [...fz.dan].filter(z => z <= PAI_ZHI.A && z > PAI_ZHI.ER).sort((a, b) => a - b);
        for (let len = 5; len <= danList.length && len === 5; len++) {
        ...
        for (const [hua, paiList] of huaSeMap) {
            if (paiList.length >= 5) {
                ...
                for (let len = 5; len <= paiList.length && len === 5; len++) {   // ★ 官方: 恰好 5 张 ✓
