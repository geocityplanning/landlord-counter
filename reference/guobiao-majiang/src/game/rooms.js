...
class Room {
  ...
  reconcileTimer() {
    ...
    if (phase === 'claiming') {
      ...
      // 认领等待时间与建房时选的出牌时间一致；无限制则不自动过
      if (this.turnTime > 0) this.timer = setTimeout(() => { this.timer = null; this.timerKind = null; try { this.game.forceResolveClaims(); } catch (e) {} this.afterChange(); }, this.turnTime);
