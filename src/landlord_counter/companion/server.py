...
class Handler(BaseHTTPRequestHandler):
    ...
    def _nocache(self) -> None:
        """★ 2026-09-22: 禁止缓存 —— 改了页面刷一下就是新的, 不用再清缓存 ✓

        用户反馈: "牌池怎么没了感觉代码版本变了 昨天不是这样的"
        查证: 页面文件早改了("0.0分"的写法当天就删了 ✓), 但浏览器**吃了缓存** ✗
              ⇒ 用户看到的是旧版 ⇒ 加这条头, 以后不存在这种误会 ✓
        """
    ...
    def _json(self, obj, code: int = 200) -> None:
        ...
        self._nocache()      # JSON 接口也不许缓存 ✓ (界面永远拿最新数据 ✓)
    ...
    def _file(self, p: Path) -> None:
        ...
        self._nocache()          # ★ 页面/静态文件都不许缓存 ✓ (改了刷新就是新的 ✓)
