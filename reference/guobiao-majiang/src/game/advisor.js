...
// 七对候选（门清无副露）：已成对的固定为对子，其余空位用单张凑对。
// 为每个单张各生成一个候选（使其成为进张），从而完整展示所有有效进张。
function qiduiCandidates(need, numMelds) {
  if (numMelds > 0) return [];
  const pairsHave = [], singles = [];
  for (let t = 1; t <= 34; t++) { if (need[t] >= 2) pairsHave.push(t); else if (need[t] === 1) singles.push(t); }
  const base = pairsHave.slice(0, 7);
  const slots = 7 - base.length;
  if (slots <= 0) { const w = new Array(35).fill(0); for (const t of base) w[t] = 2; return [{ w, cost: candCost(w, need) }]; }
  if (singles.length < slots) { const w = new Array(35).fill(0); for (const t of base.concat(singles)) w[t] = 2; return [{ w, cost: candCost(w, need) }]; }
  const out = [], seen = new Set();
  for (const s of singles) {
    const extra = [s];
    for (const x of singles) { if (extra.length >= slots) break; if (x !== s) extra.push(x); }
    const chosen = base.concat(extra);
    const key = chosen.slice().sort((a, b) => a - b).join(',');
    if (seen.has(key)) continue; seen.add(key);
    const w = new Array(35).fill(0); for (const t of chosen) w[t] = 2;
...
function analyzeHand({ hand, melds, quanfeng, menfeng }, topN = 3) {
  ...
  const special = [shisanyaoCandidate(need, numMelds)].filter(Boolean)
    .concat(qiduiCandidates(need, numMelds), quanbukaoCandidates(need, numMelds), qixingbukaoCandidates(need, numMelds), zuhelongCandidates(need, numMelds));
  let evalCount = 0;
  // budget 从 0 起：手牌若已成和（差0）就能被识别为“可胡”，而不是错误地只显示更远的目标
  for (let budget = 0; budget <= MAX_BUDGET && best.size < CAND_N && evalCount < EVAL_TOTAL; budget++) {
    const layer = [];
...
function filterByRemaining(list, remaining, topN = 3) {
  ...
  for (const it of (list || [])) {
    if (it.dist === 0) { out.push({ name: it.name, score: it.score, dist: 0, tiles: [] }); if (out.length >= topN) break; continue; } // 差0=已成和(可胡)，无需进张
