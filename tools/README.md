...
## ⚠️ 大小王（用户 2026-09-17 更正，实测踩过 ✗）

- **两张王都是彩色的** ✓，**不能靠颜色分** ✗（我曾以为"大王红/小王黑" ✗）
- 只能靠**形状**（模板）分 ✓；而**模板库里缺哪一张，它就会被读成另一张** ✗
  （实测：库里只有 16 时，小王被读成大王 ✗ → 跑一轮 `tm_collect_live.py` 补齐 `0_15` 后 100% ✓）
- 顺序：**第 1 张 = 16 = 大王，第 2 张 = 15 = 小王** ✓（与游戏源码 `XIAO_WANG:15, DA_WANG:16` + 从大到小排序一致 ✓）
